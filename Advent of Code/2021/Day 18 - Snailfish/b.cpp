...
struct Node{
	...
	Node(Node *parent, int val): val(val), parent(parent), left(nullptr), right(nullptr) {}
	...
	void dfs_before(function<void (Node *, size_t)> cb, size_t depth = 0){
		cb(this, depth);
		if (left){
	...
	void dfs_after(function<void (Node *, size_t)> cb, size_t depth = 0){
		if (left){
	...
	void reduce(){
		...
		dfs_before([&four_nested, &ten_or_greater](Node *node, size_t depth){
			...
			if (ten_or_greater == nullptr && node->is_leaf() && node->val >= 10){
				ten_or_greater = node;
			}
		});
	...
	Node *find_directly_right(){
		Node *node = this;
		while (node->parent != nullptr){
			if (node->parent->right != nullptr && node->parent->right != node){
...
struct Parser{
	...
	Parser(const string &str): str(str), i(0) {}

	char read(){
		return str[i++];
	}

	[[ noreturn ]] void syntax_err(){
		exit(1);
	}
	...
	Node *parse(Node *parent = nullptr){
		char c = read();
		if (c == '['){
