...
int getLightsOn(vector<vector<bool>>& lights){
	int count = 0;
...
void toggle_lights(int& x1, int& y1, int& x2, int& y2, vector<vector<bool>>& lights){
	for (int i = x1; i <= x2; ++i){
		for (int j = y1; j <= y2; ++j){
...
void turn_off(int& x1, int& y1, int& x2, int& y2, vector<vector<bool>>& lights){
	for (int i = x1; i <= x2; ++i){
		for (int j = y1; j <= y2; ++j){
			lights[i][j] = 0;
		}
...
void turn_on(int& x1, int& y1, int& x2, int& y2, vector<vector<bool>>& lights){
	for (int i = x1; i <= x2; ++i){
		for (int j = y1; j <= y2; ++j){
			lights[i][j] = 1;
		}
